...
Six similarity metrics ranked every gene in the dataset, surfaced their top picks, and returned confident scores. Some of those top picks looked nothing like the constraint pattern. Not even close. Yet every metric --- Pearson correlation, DTW, cosine similarity, Frechet distance, MSE, sMAPE --- reported excellent matches.

The metrics weren't buggy. They were doing exactly what they were designed to do. The problem was that what they were designed to do is not the same as what the analysis actually required.

What followed was a week of debugging that turned into a lesson about the gap between mathematical similarity and visual similarity --- and how the simplest possible fix was sitting there the whole time.
...
The [previous week's work](../six-ways-to-measure-similarity-gene-expression) implemented six similarity metrics and built a voting ensemble where each method casts votes for its top-20 genes. The ensemble worked well on some clusters and fell apart on others. On clusterThree, for instance, only one method (sMAPE) picked genes that visually followed the pattern. The other five picked genes that were confidently, demonstrably wrong.

The initial suggestion was intuitive: set score cutoffs. If a method's best score on a cluster doesn't meet some quality bar, silence it before it votes.
...
The problem reveals itself immediately in the score distributions:
...
The same method has wildly different score distributions on different clusters. A threshold tuned to silence MSE on clusterThree --- where it picks wrong genes --- also silences MSE on clusterOne, where it works fine. The score distributions are simply too different across patterns with different scales, shapes, and difficulty levels.

Pushing this to its logical extreme: what thresholds would force the "correct" answer on clusterThree (only sMAPE active)? For Pearson, it would require $r > 1.001$. That's mathematically impossible --- Pearson correlation is bounded by $[-1, 1]$. Silencing Pearson on clusterThree requires a threshold that doesn't exist.
...
The log transform (applied before normalization) compresses dynamic range but preserves the *ordering* of distances. A gene that's close stays closer than a gene that's far. Min-max normalization destroys even that. Every gene ends up in the same box, and the metrics can't tell them apart.
...
Mean-centering erases the offset. Dividing by the standard deviation erases the scale. Two genes with expression values $[0.04, 0.00, 0.037, 0.035]$ and $[1.50, 0.00, 1.30, 1.20]$ both get $r = 1.0$ against a pattern with the same shape, despite one being 37$\times$ larger.
...
*Both Gene A and Gene B have Pearson $r = 1.0$ against the constraint pattern. Gene A (green) sits directly on the pattern. Gene B (blue) is 37$\times$ larger. Pearson cannot distinguish them. Raw MAE can: Gene A has MAE $\approx 0$, Gene B has MAE $= 0.87$.*

This isn't a bug in Pearson correlation. It was designed to be scale-invariant --- that's the whole point. For the question "does this gene go up and down at the same times as the pattern," Pearson is the right tool. For the question "does this gene's expression curve *sit on top of* the pattern line on the plot," Pearson will report a perfect match with complete confidence, regardless of the actual distance.

## Act 2: Data-Driven Gating

If fixed thresholds on raw scores don't work, an alternative approach is to evaluate the *output quality* of each method --- whether its top genes actually match the pattern --- and use that to automatically silence bad methods before they vote.

Three gate candidates were tested. For each method $\times$ cluster combination, each gate produces a single quality score:
...
*Gated ensemble voting with the normalized centroid error gate. ClusterTwo and clusterThree look reasonable. ClusterOne and clusterFour still show genes far from the pattern --- gating can silence bad methods, but it cannot fix a method that was never looking at the right thing.*
...
The entire problem traces back to one decision: normalizing gene expression to $(0, 1]$ before computing similarity. The natural next step was to skip it entirely.
...
where $g$ is a gene's expression values and $p$ is the constraint pattern, both in log-transformed (but not normalized) space. This measures how far a gene's expression curve sits from the pattern line on the plot --- the closest computational analog to what the human eye does when judging a visual fit.
...
Clusters Two, Three, and Four show exactly what the analysis required: gene expression curves that hug the constraint pattern, matching both shape and magnitude. The overlap between raw MAE's top-20 and the normalized methods' top-20 was near zero for most clusters --- raw MAE finds genuinely different genes, ones that the normalized methods couldn't see.
...
If magnitude matters --- "find genes expressing at the same level as this pattern" --- then raw MAE is the right metric. It directly measures what the eye sees on a plot.

If only shape matters --- "find genes that go up and down at the same times, regardless of expression level" --- then the normalized methods with gating are the right approach. Pearson correlation's scale invariance isn't a bug; it's the feature.
